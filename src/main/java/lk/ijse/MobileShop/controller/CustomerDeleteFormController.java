...
import lk.ijse.MobileShop.bo.custom.impl.CustomerDeleteBOImpl;
...
public class CustomerDeleteFormController implements Initializable {
    ...
    CustomerDeleteBO customerDeleteBO = BOFactory.getBoFactory().getBO(BOFactory.BOType.CUSTOMER_DELETE);
    ...
    public void ComformOnAction(ActionEvent event) {
        try {
            if (customerDeleteBO.deleteCustomer(id)) {
    ...
    private void setData() {
        try {
            CustomerDTO customerDTO = customerDeleteBO.getCustomerData(id);
